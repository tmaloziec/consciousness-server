...
The ecosystem reserves the range **3030–3049** for current and
future BuildOnAI blocks. The active subset (one per block + redis)
lives in `ports.yaml` at the repo root — the single source of truth
read by every layer:

| Layer | Reads from |
|---|---|
| Native servers (Node + Python) | `lib/ports.{js,py}` → `ports.yaml` |
| `deploy/docker-compose.yml` | `${PORT_*}` interpolated from `deploy/.env` |
| `deploy/.env` | generated by `bin/sync-ports` from `ports.yaml` |
| `bin/preflight` (port collision check) | `ports.yaml` |
| Tooling like `bin/ingest-document` | `lib/ports.{py,js}` → `ports.yaml` |

**To shift the whole palette** (e.g. on a host where some defaults
are already taken): edit `ports.yaml` — every entry can move
together — then `bin/sync-ports` and you are ready for
`docker compose up`. No file in `deploy/` needs hand-editing.

`PORT` and `PORT_<NAME>` environment variables override `ports.yaml`
at the process level; useful for one-off experiments without
touching the file.

### 2.3 Categorisation — "what does this block do with data"
...
| GET | `/keys/ssh/:name` | Dispense a stored SSH key (IP-whitelist + ed25519 sig under `AUTH_MODE=enforce`) |
| GET | `/keys/api/:service` | Dispense a stored API key (IP-whitelist + ed25519 sig under `AUTH_MODE=enforce`) |
| GET | `/audit` | Recent audit entries (IP-whitelist + ed25519 sig under `AUTH_MODE=enforce`) |
...
3. Add the block's port to `ports.yaml` (under both `ports:` and
   `active_blocks:`) and run `bin/sync-ports`. The native server
   reads `ports.yaml` directly via `lib/ports.{js,py}`; compose
   interpolates `${PORT_<NAME>}` from `deploy/.env`; preflight
   picks the new entry up automatically.
4. Add a service block to `deploy/docker-compose.yml` (use any
   existing block as a template — every host-side port is already
   `${PORT_<NAME>:-default}`).
5. Register the block's URL in `services.yaml`.
6. Update `ARCHITECTURE.md` §2 (table + Categorisation).
